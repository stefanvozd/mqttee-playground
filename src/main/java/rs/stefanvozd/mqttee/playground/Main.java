...
public class Main {
	...
	private static void startLocalBroker() throws Exception {
	...
	public static void clientStub(MqttAsyncClient pcli) {
        try {
            ...
            pcli.publish("room/living/light", "on".getBytes(), 1, false);
            Thread.sleep(2000);
            pcli.publish("room/living/light", "off".getBytes(), 1, false);
            Thread.sleep(2000);
            pcli.publish("room/living/light", "on".getBytes(), 1, false);
